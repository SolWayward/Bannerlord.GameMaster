...
public static class GiveXpCommand
{
    ...
    public static string GiveXp(List<string> args)
    {
        return Cmd.Run(args, () =>
        {
            ...
            if (xpArg == null)
                return CommandResult.Error(MessageFormatter.FormatErrorMessage("Missing required argument 'xp'.")).Message;

            if (!CommandValidator.ValidateIntegerRange(xpArg, 1, 1000000, out int xp, out string xpError))
                return CommandResult.Error(MessageFormatter.FormatErrorMessage(xpError)).Message;
