...
public static class MakePeaceCommand
{
    ...
    public static string MakePeace(List<string> args)
    {
        return Cmd.Run(args, () =>
        {
            ...
            if (kingdom1 == kingdom2)
                return MessageFormatter.FormatErrorMessage("A kingdom cannot make peace with itself.");

            if (!FactionManager.IsAtWarAgainstFaction(kingdom1, kingdom2))
                return MessageFormatter.FormatErrorMessage($"{kingdom1.Name} and {kingdom2.Name} are not at war.");
